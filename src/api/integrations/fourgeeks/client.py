...
class FourGeeksClient:
    ...
    def get_student_by_email(self, email: str) -> Dict[str, Any]:
        ...
        try:
            response = self._client.get(
                f"{self.BASE_URL}/admissions/student",
                params={"like": email},
                headers=self._get_headers()
            )
            response.raise_for_status()
            ...
            logging.error(f"HTTP error occurred in FourGeeksClient get_student_by_email: {e}")
            raise HTTPException(
                status_code=500, detail=f"HTTP error occurred in FourGeeksClient get_student_by_email: {e}")
        except Exception as e:
            logging.error(f"Error occurred in FourGeeksClient get_student_by_email: {e}")
            raise HTTPException(
    ...
    def get_cohort(self, cohort_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.error(f"HTTP error occurred in FourGeeksClient get_cohort: {e}")
            raise HTTPException(
    ...
    def get_cohort_user(self, cohort_id: int, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.error(f"HTTP error occurred in FourGeeksClient get_cohort_user: {e}")
            raise HTTPException(
                status_code=500, detail=f"HTTP error occurred in FourGeeksClient get_cohort_user: {e}")
        except Exception as e:
            logging.error(f"Error occurred in FourGeeksClient get_cohort_user: {e}")
            raise HTTPException(
