"""Add program_type field to Service model
...
def upgrade() -> None:
    ...
    op.alter_column('contractaccrual', 'updated_at',
               ...
               type_=sa.DateTime(timezone=True),
               existing_nullable=False)
    op.add_column('service', sa.Column('program_type', sqlmodel.sql.sqltypes.AutoString(), nullable=True))
    op.create_index(op.f('ix_service_program_type'), 'service', ['program_type'], unique=False)
    ...
    # --- Prefill program_type for existing services ---
    connection = op.get_bind()
    
    # Minimal inlined logic for program type extraction
    def classify_program_type(service_name):
        if not service_name:
            return "UNKNOWN"
    ...
    for service in services:
        program_type = classify_program_type(service.name)
        connection.execute(
            sa.text("UPDATE service SET program_type = :program_type WHERE id = :id"),
            {"program_type": program_type, "id": service.id}
        )
    # --- End prefill ---
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_service_program_type'), table_name='service')
    op.drop_column('service', 'program_type')
    op.alter_column('contractaccrual', 'updated_at',
